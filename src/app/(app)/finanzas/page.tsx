...
import { revenueByMonth, serviceOrders } from "@/lib/demo-data";
import { serviceTypeLabel } from "@/lib/labels";
import { formatRD } from "@/lib/utils";

const expenseByCategory = [
  { name: "Materiales", value: 342_000 },
  { name: "Nómina", value: 185_000 },
  { name: "Transporte", value: 48_000 },
  { name: "Herramientas", value: 27_000 },
];

const PIE_COLORS = ["#ffcc00", "#e6b800", "#b38f00", "#806600"];

export default function FinanzasPage() {
  const totalIngresos = revenueByMonth.reduce((s, m) => s + m.ingresos, 0);
  const totalGastos = revenueByMonth.reduce((s, m) => s + m.gastos, 0);
  const ganancia = totalIngresos - totalGastos;

  // Ingresos por tipo de servicio (desde las órdenes).
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Últimos 6 meses · comparativo ingresos vs gastos
            </p>
        ...
        <StaggerItem>
          <div className="glass-card p-5">
            <div className="mb-2 flex items-center gap-2 text-slate-400">
              <DollarSign className="h-4 w-4" /> <span className="text-xs">Ingresos (6m)</span>
            </div>
            <div className="kpi-value text-emerald-600 dark:text-emerald-400">
              <CountUp value={totalIngresos} format={(n) => formatRD(n, { decimals: false })} />
            </div>
          </div>
        </StaggerItem>
        <StaggerItem>
          <div className="glass-card p-5">
            <div className="mb-2 flex items-center gap-2 text-slate-400">
              <Wallet className="h-4 w-4" /> <span className="text-xs">Gastos (6m)</span>
            </div>
            <div className="kpi-value text-red-500 dark:text-red-400">
              <CountUp value={totalGastos} format={(n) => formatRD(n, { decimals: false })} />
            </div>
          </div>
        </StaggerItem>
        <StaggerItem>
          <div className="glass-card p-5">
            <div className="mb-2 flex items-center gap-2 text-slate-400">
              <PiggyBank className="h-4 w-4" /> <span className="text-xs">Ganancia neta</span>
            </div>
            <div className="kpi-value text-volt-600 dark:text-volt-400">
              <CountUp value={ganancia} format={(n) => formatRD(n, { decimals: false })} />
            </div>
          </div>
        </StaggerItem>
      </Stagger>
        ...
        <div className="glass-card p-5">
          ...
          <div className="h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={revenueByMonth} margin={{ top: 6, right: 6, left: -12, bottom: 0 }}>
                <defs>
    ...
    <div className="rounded-xl border border-slate-200 bg-white/95 px-3 py-2 text-xs shadow-lg backdrop-blur dark:border-white/10 dark:bg-ink-800/95">
      ...
      {payload.map((p: any, i: number) => (
        <p key={i} className="tabular-nums" style={{ color: p.color ?? p.fill }}>
          {p.name}: {formatRD(p.value, { decimals: false })}
        </p>
      ))}
    </div>
  );
