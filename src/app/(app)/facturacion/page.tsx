        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
          <div className="glass-card p-4">
            ...
            <p className="mt-1 text-xl font-bold tabular-nums text-emerald-600 dark:text-emerald-400">
              {formatRD(paidTotal, { decimals: false })}
            </p>
          </div>
          <div className="glass-card p-4">
            ...
            <p className="mt-1 text-xl font-bold tabular-nums text-amber-600 dark:text-amber-400">
              {formatRD(pendingTotal, { decimals: false })}
            </p>
          </div>
          <div className="glass-card col-span-2 p-4 sm:col-span-1">
            <p className="text-xs text-slate-400">ITBIS acumulado (18%)</p>
            <p className="mt-1 text-xl font-bold tabular-nums">
              {formatRD(invoices.reduce((s, i) => s + i.itbis, 0), { decimals: false })}
            </p>
          </div>
        </div>
      </Reveal>
            ...
            >
              <div className="flex flex-wrap items-center gap-4">
                <div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-volt-500/10 ring-1 ring-inset ring-volt-500/20">
                  <Receipt className="h-5 w-5 text-volt-500" />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge className={invoiceStatusStyle[inv.status]}>
                      {invoiceStatusLabel[inv.status]}
                    </Badge>
                  </div>
                  <h3 className="mt-0.5 truncate font-semibold">{inv.clientName}</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    NCF {inv.ncf} · {formatDate(inv.createdAt)}
